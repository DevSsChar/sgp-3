...
const BG_PARTICLE_DENSITY = 0.00005; // Background particles
const MOUSE_RADIUS = 180; // Radius of mouse influence
const RETURN_SPEED = 0.08; // Spring constant for return to origin
const DAMPING = 0.90; // Friction (velocity decay)
const REPULSION_STRENGTH = 1.2; // Mouse push force multiplier
...
const ParticleBackground = forwardRef((props, ref) => {
  ...
  // Expose mouse control methods to parent
  ...
  const initParticles = useCallback((width, height) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        ...
        color: Math.random() > 0.9 ? '#B0B0B0' : '#ffffff', // Grayscale theme
  ...
  const animate = useCallback((time) => {
    ...
    // 1. Pulsating Radial Glow (Grayscale)
    ...
    gradient.addColorStop(0, `rgba(117, 117, 117, ${pulseOpacity})`); // Gray glow
